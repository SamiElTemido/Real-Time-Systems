...
/* Includes de FreeRTOS añadidos */
#include "FreeRTOS.h"
#include "task.h"

/* Prototipo de Error_Handler (definido en main_init.c) */
extern void Error_Handler(void);

/* Definimos un timeout razonable para las operaciones I2C/SPI */
#define SSD1306_TIMEOUT 100

#define SSD1306_USE_I2C
extern I2C_HandleTypeDef hi2c1;
#define SSD1306_I2C_PORT   hi2c1
#define SSD1306_I2C_ADDR   (0x3C << 1)   // la mayoría de módulos I2C SSD1306
...
void ssd1306_WriteCommand(uint8_t byte) {
    /* CAMBIO: Reemplazado HAL_MAX_DELAY con un timeout Y AÑADIDA COMPROBACIÓN DE ERRORES */
    if (HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x00, 1, &byte, 1, SSD1306_TIMEOUT) != HAL_OK) {
        Error_Handler();
    }
}
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    /* CAMBIO: Reemplazado HAL_MAX_DELAY con un timeout Y AÑADIDA COMPROBACIÓN DE ERRORES */
    if (HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1, buffer, buff_size, SSD1306_TIMEOUT) != HAL_OK) {
        Error_Handler();
    }
}
...
void ssd1306_Reset(void) {
    ...
    /* CAMBIO: Reemplazado HAL_Delay por vTaskDelay */
...
void ssd1306_WriteCommand(uint8_t byte) {
    ...
    /* CAMBIO: Reemplazado HAL_MAX_DELAY con un timeout */
    if (HAL_SPI_Transmit(&SSD1306_SPI_PORT, (uint8_t *) &byte, 1, SSD1306_TIMEOUT) != HAL_OK) {
        Error_Handler();
    }
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    ...
    /* CAMBIO: Reemplazado HAL_MAX_DELAY con un timeout */
    if (HAL_SPI_Transmit(&SSD1306_SPI_PORT, buffer, buff_size, SSD1306_TIMEOUT) != HAL_OK) {
        Error_Handler();
    }
    HAL_GPIO_WritePin(SSD1306_CS_Port, SSD1306_CS_Pin, GPIO_PIN_SET); // un-select OLED
}

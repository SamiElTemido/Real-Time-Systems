...
void ssd1306_TestBorder() {
    ...
    do {
        ...
        vTaskDelay(pdMS_TO_TICKS(5)); // <--- CAMBIO
    } while(x > 0 || y > 0);
   
    vTaskDelay(pdMS_TO_TICKS(1000)); // <--- CAMBIO
}
...
void ssd1306_TestFPS() {
    ...
    uint32_t start = xTaskGetTickCount(); // <--- CAMBIO
    uint32_t end = start;
    int fps = 0;
    ...
    do {
        ...
        fps++;
        end = xTaskGetTickCount(); // <--- CAMBIO
    } while((end - start) < 5000); // <--- CAMBIO (para comparar ticks con ticks)
   
    vTaskDelay(pdMS_TO_TICKS(5000)); // <--- CAMBIO
...
void ssd1306_TestDrawBitmap()
{
    ...
    ssd1306_UpdateScreen();
    vTaskDelay(pdMS_TO_TICKS(3000)); // <--- CAMBIO
}

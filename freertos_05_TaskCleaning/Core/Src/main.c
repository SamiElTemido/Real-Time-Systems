#include "main.h"

UART_HandleTypeDef huart1 = {0};
TaskHandle_t task1;
TaskHandle_t task2;

void myFirstTask(void *);
void secondTask(void *);
void thirdTask(void *);

const char *msg1 = "hello real-time world task 1\n\r";
const char *msg2 = "hello real-time world task 2\n\r";

int main(void)
{
	System_Init();

	xTaskCreate(myFirstTask, "FirstTask", configMINIMAL_STACK_SIZE, NULL, 0, NULL);
	xTaskCreate(secondTask, "SecondTask", configMINIMAL_STACK_SIZE, (void *)msg1, 1, &task1);
	xTaskCreate(thirdTask, "ThirdTask", configMINIMAL_STACK_SIZE, (void *)msg2, 0, &task2);

	vTaskStartScheduler();
	return 0;
}

/* Alterna el LED y cambia qué tarea de mensaje está activa */
void myFirstTask(void *pvParameters)
{
	(void)pvParameters;
	uint8_t flag = 0;

	while (1)
	{
		HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
		vTaskDelay(pdMS_TO_TICKS(1000));

		if (!flag)
		{
			vTaskSuspend(task1);
			vTaskResume(task2);
		}
		else
		{
			vTaskSuspend(task2);
			vTaskResume(task1);
		}
		flag = !flag;
	}
}

/* Envía msg1 por UART y prioriza task2 */
void secondTask(void *pvParameters)
{
	char *msg = (char *)pvParameters;

	while (1)
	{
		HAL_UART_Transmit(&huart1, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
		vTaskDelay(pdMS_TO_TICKS(1000));
		vTaskPrioritySet(task2, 1);
		vTaskPrioritySet(task1, 0);
	}
}

/* Envía msg2 por UART y prioriza task1 */
void thirdTask(void *pvParameters)
{
	char *msg = (char *)pvParameters;

	while (1)
	{
		HAL_UART_Transmit(&huart1, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
		vTaskDelay(pdMS_TO_TICKS(1000));
		vTaskPrioritySet(task1, 1);
		vTaskPrioritySet(task2, 0);
	}
}

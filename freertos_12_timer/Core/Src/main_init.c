...
void GPIO_Init(void){
	//habilitas reloj
		...
		pin.Pin = DIR_A_PIN|DIR_B_PIN;				// PINES DE DIRECCION POR AHORA B7
		pin.Mode= GPIO_MODE_OUTPUT_PP;
		pin.Pull =GPIO_NOPULL;
		HAL_GPIO_Init(GPIOB, &pin);
		HAL_GPIO_WritePin(DIR_GPIO, DIR_A_PIN | DIR_B_PIN, GPIO_PIN_RESET);
}
...
void TIM2_Init(void){
   ...
   qei_conf.EncoderMode = TIM_ENCODERMODE_TI12;

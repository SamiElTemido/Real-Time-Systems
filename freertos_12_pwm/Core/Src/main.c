...
void control_task(void *pvParameter)
{
    ...
    // Variable para llevar la cuenta del valor del PWM
    uint16_t pwm_value = 0;

    // Configuración para que la tarea se ejecute cada 1 ms
    TickType_t xLastWakeTime = xTaskGetTickCount();
    const TickType_t xPeriod = pdMS_TO_TICKS(10);

    // Se establece una dirección de giro fija ANTES de entrar al bucle
    HAL_GPIO_WritePin(DIR_GPIO, DIR_A_PIN, GPIO_PIN_SET);
    HAL_GPIO_WritePin(DIR_GPIO, DIR_B_PIN, GPIO_PIN_RESET); // Sentido de giro único

    while (1)
    {
        // Espera hasta el siguiente ciclo de ejecución
        vTaskDelayUntil(&xLastWakeTime, xPeriod);

        // --- Lógica de Rampa de PWM ---

        // Incrementa el valor del PWM en cada ciclo
        pwm_value++;

        // Si el valor supera 999, se reinicia a 0 para empezar de nuevo el barrido
        if (pwm_value > 999)
        {
            pwm_value = 0;
        }

        // Aplica el valor actual del PWM al Timer 4, Canal 1
